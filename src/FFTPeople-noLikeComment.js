...
const instagramBot = async (userId) => {
  const botTask = [botFollow(userId)];
  const [Follow] = await Promise.all(botTask);
  const botFollowStatus = Follow ? chalk`{green Follow}` : chalk`{red Follow}`;
  return chalk`{bold.green ${botFollowStatus}}`;
};
        ...
        await Promise.all(
          getTargetFollowers[i].map(async (account) => {
            if (!getMyFollowers.includes(account.pk) && account.is_private === false) {
              const runBot = await instagramBot(account.pk);
