...
const User = [
  ...
  },
  {
    type: "input",
    name: "sleep",
    message: "[>] Insert Sleep/Delay (Seconds):",
    validate: function (value) {
      value = value.match(/[0-9]/);
      if (value) return true;
...
const instagramBot = async (userId, text) => {
  ...
  if (getUserItems.length > 0) {
    ...
    const botTask = [botFollow(userId), botComment(latestPost.id, text), botLike(latestPost.id)];
    const [Follow, Comment, Like] = await Promise.all(botTask);
    ...
    return chalk`{bold.green ${botFollowStatus},${botCommentStatus},${botLikeStatus}, [${text}]}`;
  }
