...
const instagramBot = async (userId, text) => {
  ...
  if (getUserItems.length > 0) {
    ...
    const botTask = [botFollow(userId), botComment(latestPost.id, text), botLike(latestPost.id), botDM(userId, text)];
    const [Follow, Comment, Like, DM] = await Promise.all(botTask);
    ...
    let botLikeStatus = Like ? chalk`{green Like}` : chalk`{red Like}`;
    let botDMStatus = DM ? chalk`{green DM}` : chalk`{red DM}`;
    return chalk`{bold.green ${botFollowStatus},${botCommentStatus},${botLikeStatus},${botDMStatus}, [${text}]}`;
  }
