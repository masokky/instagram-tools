...
const instagramBot = async (userId, text) => {
  ...
  if (getUserItems.length > 0) {
    ...
    if (latestPost.has_liked) {
      return chalk`{bold.blue Already Liked & Comment}`;
    }
    const botTask = [botComment(latestPost.id, text), botLike(latestPost.id)];
    const [Comment, Like] = await Promise.all(botTask);
    let botCommentStatus = Comment ? chalk`{green Comment}` : chalk`{red Comment}`;
    let botLikeStatus = Like ? chalk`{green Like}` : chalk`{red Like}`;
    return chalk`{bold.green ${botCommentStatus},${botLikeStatus}, [${text}]}`;
  }
        ...
        await Promise.all(
          getTargetFollowers[i].map(async (account) => {
            if (!getMyFollowers.includes(account.pk) && account.is_private === false) {
              ...
              console.log(chalk`[{magenta ${timeNow}}] {bold.yellow [SKIP]}${account.username} => PRIVATE`);
            }
          })
        );
