...
const json_pruned_1 = require("./json-pruned");
...
class Media extends resource_1.InstagramResource {
    ...
    static async likers(session, mediaId) {
        const data = await new core_1.Request(session)
            ...
            .send();
        return class_transformer_1.plainToClass(responses_1.UserResponse, data.users);
    }
    ...
    static configurePhoto(session, uploadId, caption, width, height, userTags, location) {
        ...
        return session
            .getAccountId()
            .then(accountId => {
            let payload = json_pruned_1.pruned({
                source_type: '4',
                caption,
                upload_id: uploadId,
    ...
    static configureVideo(session, uploadId, caption, durationms, delay, { audio_muted = false, trim_type = 0, source_type = 'camera', mas_opt_in = 'NOT_PROMPTED', disable_comments = false, filter_type = 0, poster_frame_index = 0, geotag_enabled = false, camera_position = 'unknown', } = {}) {
        ...
        return Bluebird.delay(delay)
            ...
            .then(accountId => {
            const payload = json_pruned_1.pruned({
    ...
    static configureAlbum(session, medias, caption, disableComments) {
        ...
        return Bluebird.mapSeries(medias, media => {
            if (media.type === 'photo') {
    ...
    parseParams(json) {
        ...
        if (_.isObject(json.location)) {
            ...
            this.location = new location_1.Location(that.session, location);
        }
        ...
        this.previewComments = _.map(json.preview_comments, comment => new comment_1.Comment(that.session, comment));
        this.comments = this.previewComments;
        if (_.isPlainObject(json.user))
            this.account = class_transformer_1.plainToClass(responses_1.UserResponse, json.user);
        return hash;
    }
    getParams() {
